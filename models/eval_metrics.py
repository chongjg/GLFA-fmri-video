...
def img_classify_metric(
                        ...
                        num_trials: int = 100,
                        top_k: int = 1,
                        cache_dir: str = '/home/lichong/.cache/huggingface/hub/',
                        device: Optional[str] = 'cuda',
    ...
    acc_list = [[] for i in range(len(n_way))]
    std_list = [[] for i in range(len(n_way))]
    for pred, gt in zip(pred_videos, gt_videos):
        pred = processor(images=pred.astype(np.uint8), return_tensors='pt')['pixel_values']
        gt = processor(images=gt.astype(np.uint8), return_tensors='pt')['pixel_values']
...
def video_classify_metric(
    ...
    for pred, gt in zip(pred_videos, gt_videos):
        pred = processor(list(pred), return_tensors='pt')['pixel_values']
        gt = processor(list(gt), return_tensors='pt')['pixel_values']

        gt_class_id = model(gt.to(device).to(torch.float16)).logits.argsort(-1,descending=False).detach().flatten()[-3:]
        pred_out = model(pred.to(device).to(torch.float16)).logits.softmax(-1).detach().flatten()
