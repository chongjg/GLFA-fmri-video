...
def main(
    ...
    for i  in range(400):
        if i % 50 == 0:
            print(f'{i} / 400')
        gif = iio.imread(os.path.join(data_path, f'samples/fmri-dataset-test-{i}.mp4'), index=None)
...
if __name__ == '__main__':
    main(
        data_path = sys.argv[1] #'./outputs-inference/fmri-video-sub1-10k/'
    )

# CUDA_VISIBLE_DEVICES=0 python -u run_metrics.py ./outputs-inference/fmri-video-sub12-funcalign-22k/ > ./outputs-inference/fmri-video-sub12-funcalign-22k/test.out
